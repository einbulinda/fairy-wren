...
const BetaOpenBillsModal = ({ bills, onSelectBill, onClose, title }) => {
  const getBillPayment = (bill) => {
    try {
      return calculateBillPaymentInfo(bill);
    } catch (error) {
      console.error("Error calculating bill totals:", error, bill);
      return { total: 0, balanceDue: 0, amountPaid: 0 };
    }
  };
      ...
      <div className="w-full lg:max-w-md bg-slate-900 rounded-t-2xl lg:rounded-2xl border border-slate-700/50 shadow-2xl max-h-[80vh] lg:max-h-[600px] flex flex-col">
        ...
        <div className="p-4 border-b border-slate-700/30 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-400">
              {billsList.length} bill{billsList.length !== 1 ? "s" : ""}
              {billsList.length > 0 && (
                <span className="ml-2 text-pink-400 font-semibold">
                  · Outstanding: KSh {billsList.reduce((sum, b) => sum + getBillPayment(b).balanceDue, 0).toLocaleString("en-KE", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                </span>
              )}
            </p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2">
          {billsList.length === 0 ? (
            ...
            billsList.map((bill) => {
              const { total, balanceDue, amountPaid } = getBillPayment(bill);
                    ...
                    <div className="flex-1 min-w-0 pr-3">
                      ...
                      {hasPartialPayment && (
                        <p className="text-xs text-gray-500 mt-1">
                          Bill: KSh {total.toLocaleString()} · Paid: <span className="text-emerald-400">KSh {amountPaid.toLocaleString()}</span>
                        </p>
                      )}
                    </div>
                    <div className="text-right shrink-0">
                      <p className="text-xl font-bold bg-linear-to-r from-pink-400 to-purple-400 bg-clip-text text-transparent">
