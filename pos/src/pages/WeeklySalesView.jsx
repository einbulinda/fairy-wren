...
import { TrendingUp, TrendingDown, Minus, BarChart3, ShoppingBag, Receipt, Calendar } from "lucide-react";
...
const usePeriodSales = (startDate, endDate) => {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    setLoading(true);
    BillsService.list({ startDate, endDate, status: "paid", limit: 1000 })
      .then(({ bills }) => {
        if (!cancelled) setData(bills || []);
      })
      .catch(() => {
...
const DeltaBadge = ({ current, prev, suffix = "" }) => {
...
const StatCard = ({ icon: Icon, label, current, prev, format }) => {
  ...
  return (
    <div className="bg-gray-800/60 border border-purple-500/20 rounded-xl p-4 flex flex-col gap-2">
      ...
      <p className="text-white text-2xl font-bold">{display}</p>
...
const WeeklySalesView = () => {
  ...
  const thisStart = toDateStr(thisMonday);
  const thisEnd = toDateStr(today);
  ...
  return (
    <div className="p-4 space-y-6 overflow-y-auto h-full pb-24 md:pb-4">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-white font-bold text-lg">Weekly Sales</h2>
        ...
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            <StatCard
              icon={Receipt}
              label="Revenue"
              current={thisSummary?.revenue ?? 0}
              prev={prevSummary?.revenue ?? 0}
              format={fmt}
            />
            <StatCard
              icon={ShoppingBag}
              label="Bills"
              current={thisSummary?.count ?? 0}
              prev={prevSummary?.count ?? 0}
            />
            <StatCard
              icon={TrendingUp}
              label="Avg Bill"
              current={thisSummary?.avg ?? 0}
              prev={prevSummary?.avg ?? 0}
              format={fmt}
            />
          </div>
                ...
                return (
                  <div key={day.date} className="flex-1 flex flex-col items-center gap-1">
                    <div className="w-full flex flex-col justify-end" style={{ height: "96px" }}>
                      <div
                        className={`w-full rounded-t transition-all ${isToday ? "bg-purple-500" : "bg-purple-800/60"}`}
              ...
              <tbody className="divide-y divide-gray-700/50">
                {dayBreakdown.map((day) => {
                  ...
                  return (
                    <tr
                      key={day.date}
                      className={`transition-colors ${isToday ? "bg-purple-500/10" : "hover:bg-gray-700/30"}`}
                    >
                      <td className={`px-4 py-3 font-medium ${isToday ? "text-purple-300" : "text-gray-300"}`}>
                        {day.label}
                        {isToday && <span className="ml-2 text-xs text-purple-400">(today)</span>}
                      </td>
                      <td className="px-4 py-3 text-right text-gray-400">{day.count}</td>
          ...
          <div className="bg-gray-800/60 border border-purple-500/20 rounded-xl p-4">
            <p className="text-xs text-gray-400 uppercase tracking-wide mb-3">vs Previous Week ({prevStart} — {prevEnd})</p>
