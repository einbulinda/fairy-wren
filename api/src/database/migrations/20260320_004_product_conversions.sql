...
-- Index for lookups
CREATE INDEX IF NOT EXISTS idx_product_conversions_source
  ON public.product_conversions(source_product_id);
CREATE INDEX IF NOT EXISTS idx_product_conversions_target
  ON public.product_conversions(target_product_id);
...
BEGIN
  ...
  -- Check available stock (from inventory_on_hand view or sum movements)
  SELECT COALESCE(SUM(
  ...
  -- Create conversion_out movement (deduct from source)
