...
CREATE TABLE IF NOT EXISTS business_targets (
    ...
    target_gross_margin DECIMAL(5, 2) NOT NULL DEFAULT 35.00,
    -- percentage
    target_net_margin DECIMAL(5, 2) DEFAULT NULL,
    -- optional
    -- Cash flow targets
    target_cash_reserve_days INTEGER DEFAULT 30,
    max_outstanding_ratio DECIMAL(5, 2) DEFAULT 20.00,
    -- % of monthly revenue
    -- Operational targets
    ...
    SET NULL,
        ...
        updated_by UUID REFERENCES profiles(id) ON DELETE
    SET NULL,
...
CREATE TRIGGER update_business_targets_updated_at BEFORE
UPDATE ON business_targets FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
DROP TRIGGER IF EXISTS update_user_targets_updated_at ON user_targets;
CREATE TRIGGER update_user_targets_updated_at BEFORE
UPDATE ON user_targets FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
DROP TRIGGER IF EXISTS update_category_targets_updated_at ON category_targets;
CREATE TRIGGER update_category_targets_updated_at BEFORE
UPDATE ON category_targets FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
SELECT EXTRACT(
    ...
    1000000.00 as target_revenue,
    -- Default 1M KES
    35.00 as target_gross_margin,
    -- Default 35%
    NOW() as created_at
