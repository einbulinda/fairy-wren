...
exports.getPreviousMonthRevenue = async (year, month) => {
  ...
  return data.reduce((sum, bill) => sum + (parseFloat(bill.bill_total) || 0), 0);
};
...
exports.getDashboardMetricsWithTargets = async (startDate, endDate) => {
  ...
  const totalRevenue = completedBills.reduce((sum, b) => sum + (parseFloat(b.bill_total) || 0), 0);
  ...
  if (!targetRevenue) {
    ...
    targetRevenue = Math.round(prevRevenue * 1.10); // 10% growth
