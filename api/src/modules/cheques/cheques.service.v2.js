...
exports.create = async (payload, context) => {
  ...
  await invalidateFinancialCaches([dto.bank_account_id, dto.debit_account_id]);
  ...
  return exports.getDetailed(cheque.id);
};
...
exports.void = async (id, context) => {
  ...
  if (cheque.journal_entry_id) {
    ...
    if (original && original.journal_lines) {
  ...
  if (affectedAccounts.length > 0) {
    await invalidateFinancialCaches(affectedAccounts);
  }
