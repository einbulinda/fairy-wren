...
import { useProductInsights, useProductPurchaseHistory, useProductSalesHistory } from "@/hooks/useProducts";
...
const fmtD = (n) => Number(n || 0).toLocaleString("en-KE", { minimumFractionDigits: 2, maximumFractionDigits: 2 });
const fmtDate = (d) => d ? new Date(d).toLocaleDateString("en-KE", { day: "2-digit", month: "short", year: "numeric" }) : "—";

// ─── Metric Card ──────────────────────────────────────────────────────────────

const MetricCard = ({ label, value, sub, icon: Icon, color = "primary", trend }) => (
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <MetricCard
          ...
          icon={Package}
          color={metrics?.current_stock === 0 ? "red" : metrics?.current_stock <= 5 ? "yellow" : "green"}
        />
        <MetricCard
          label="Total Revenue"
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
        <div className="bg-surface-800 rounded-xl border border-surface-700 p-4">
          ...
          <p className="text-lg font-bold text-white">{fmt(metrics?.total_units_purchased)}</p>
          <p className="text-surface-500 text-xs mt-1">Avg cost KSh {fmtD(metrics?.avg_cost_price)}</p>
        </div>
        <div className="bg-surface-800 rounded-xl border border-surface-700 p-4">
          <p className="text-surface-400 text-xs mb-2">Cost of Goods Sold</p>
          <p className="text-lg font-bold text-white">KSh {fmt(metrics?.total_cogs)}</p>
          <p className="text-surface-500 text-xs mt-1">Based on avg cost price</p>
        </div>
        <div className={`bg-surface-800 rounded-xl border p-4 ${
          metrics?.days_of_stock_remaining != null && metrics.days_of_stock_remaining <= 7
      ...
      <div className="bg-surface-800 rounded-xl border border-surface-700 p-5">
        ...
        <dl className="grid grid-cols-2 sm:grid-cols-3 gap-4 text-sm">
          {[
            ...
            { label: "Cost Price",    value: product?.cost_price ? `KSh ${fmtD(product.cost_price)}` : "Not set" },
...
const PurchasesTab = ({ productId }) => {
  ...
  if (purchases.length === 0) return <p className="text-surface-400 text-sm py-4">No purchase records found.</p>;
      ...
      <table className="w-full text-sm">
        <thead className="bg-surface-900 text-surface-400 uppercase text-xs">
          <tr>
            ...
            <th className="px-4 py-3 text-left hidden sm:table-cell">Invoice</th>
        ...
        <tbody className="divide-y divide-surface-700">
          {purchases.map((item) => (
            <tr key={item.id} className="hover:bg-surface-700/50 transition-colors">
              ...
              <td className="px-4 py-3 text-right font-mono text-white">{item.quantity}</td>
              <td className="px-4 py-3 text-right font-mono text-surface-300">KSh {fmtD(item.unit_cost)}</td>
              <td className="px-4 py-3 text-right font-mono font-semibold text-primary-400">KSh {fmtD(item.line_total)}</td>
            </tr>
          ))}
        </tbody>
        <tfoot className="bg-surface-900 font-semibold">
          <tr>
            <td colSpan={3} className="px-4 py-2 text-surface-400 text-xs">Total</td>
...
const SalesTab = ({ productId }) => {
  ...
  if (isLoading) return <p className="text-surface-400 text-sm py-4">Loading…</p>;
  if (sales.length === 0) return <p className="text-surface-400 text-sm py-4">No sales records found.</p>;
  ...
  return (
    <div className="bg-surface-800 rounded-xl border border-surface-700 overflow-hidden">
      <table className="w-full text-sm">
        ...
        <tbody className="divide-y divide-surface-700">
          {sales.map((item) => {
            const saleDate = item.rounds?.created_at || item.rounds?.bills?.created_at;
            const revenue = (item.quantity || 0) * (item.price || 0);
            return (
              <tr key={item.id} className="hover:bg-surface-700/50 transition-colors">
                <td className="px-4 py-3 text-surface-300 whitespace-nowrap">{fmtDate(saleDate)}</td>
                <td className="px-4 py-3 text-surface-400 hidden sm:table-cell">
                  Round #{item.rounds?.round_number ?? "—"}
                </td>
                <td className="px-4 py-3 text-right font-mono text-white">{item.quantity}</td>
                <td className="px-4 py-3 text-right font-mono text-surface-300">KSh {fmtD(item.price)}</td>
                <td className="px-4 py-3 text-right font-mono font-semibold text-green-400">KSh {fmtD(revenue)}</td>
              </tr>
            );
          })}
        </tbody>
        <tfoot className="bg-surface-900 font-semibold">
          <tr>
            <td colSpan={2} className="px-4 py-2 text-surface-400 text-xs">Total</td>
            <td className="px-4 py-2 text-right font-mono text-white text-xs">
              {sales.reduce((s, r) => s + (r.quantity || 0), 0)}
            </td>
            <td />
            <td className="px-4 py-2 text-right font-mono text-green-400 text-xs">
              KSh {fmtD(sales.reduce((s, r) => s + (r.quantity || 0) * (r.price || 0), 0))}
            </td>
          </tr>
        </tfoot>
      </table>
  ...
  if (!product) {
    return (
      <div className="flex flex-col items-center justify-center h-48 gap-3">
        ...
        <button onClick={() => navigate("/products")} className="text-primary-400 hover:text-primary-300 text-sm">← Back to Products</button>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-start gap-3">
        <button onClick={() => navigate("/products")} className="mt-1 p-1.5 hover:bg-surface-700 rounded-lg text-surface-400 hover:text-white transition-colors">
          <ArrowLeft size={18} />
        </button>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-3">
            ...
            <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${
              product.active ? "bg-green-500/20 text-green-400" : "bg-surface-700 text-surface-400"
            }`}>
        ...
        <div className="text-right shrink-0">
          <p className="text-2xl font-bold text-primary-400">KSh {fmtD(product.price)}</p>
        ...
        {TABS.map(({ id: tid, label, icon: Icon }) => (
          <button
            ...
            className={`flex items-center gap-2 px-4 py-3 text-sm font-medium transition-all relative ${
              activeTab === tid ? "text-primary-400" : "text-surface-400 hover:text-surface-200"
            }`}
          >
            <Icon size={15} />
            <span className="hidden sm:inline">{label}</span>
            {activeTab === tid && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-primary-500" />}
          </button>
        ))}
      </div>

      {activeTab === TAB_OVERVIEW  && <OverviewTab product={product} metrics={metrics} />}
