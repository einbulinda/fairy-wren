      ...
      <div className="bg-surface-800 rounded-xl border border-surface-700 p-5 space-y-4">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="flex items-center gap-1 px-3 py-1.5 bg-primary-600 hover:bg-primary-700 rounded-lg text-sm text-white transition-colors"
          >
          ...
          <div className="relative">
            ...
            {productSearch && (
              <div className="absolute z-10 w-full mt-1 bg-surface-900 border border-surface-600 rounded-lg shadow-xl max-h-52 overflow-y-auto">
                {filtered.length === 0 ? (
                  <p className="px-4 py-3 text-surface-400 text-sm">
                    No products found
                  </p>
                ) : (
                  filtered.map((p) => (
                    <button
                      ...
                      type="button"
                      onClick={() => addLine(p)}
                      className="w-full px-4 py-2.5 text-left hover:bg-surface-700 text-sm border-b border-surface-700 last:border-0"
                    >
  ...
  return (
    <div className="space-y-3">
      {reports.map((r) => (
        <div
          key={r.id}
          className="bg-surface-800 rounded-xl border border-surface-700 p-4"
        >
          <div className="flex items-center justify-between mb-3">
            <div>
              ...
              <p className="text-xs text-surface-400">
                By {r.completed_by_name || r.profiles?.name || "—"}
              </p>
            </div>
            <span
          ...
          {r.items && (
            <div className="text-xs text-surface-400 flex gap-4">
              ...
              <span>
                {r.items.filter((i) => i.variance !== 0).length} variances
              </span>
            </div>
          )}
        </div>
...
const ApprovalsTab = () => {
  ...
  const pending = (adjustments || []).filter(
    (r) => r.status === "completed" || r.status === "pending_approval",
  );
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-xs text-surface-400">
                {session.items?.length || 0} items counted
              </p>
            </div>
            <div className="flex gap-2">
              <button
                onClick={() =>
                  approveMutation.mutate(session.id, { onSuccess: refetch })
                }
              ...
              <button
                onClick={() =>
                  rejectMutation.mutate(session.id, { onSuccess: refetch })
                }
