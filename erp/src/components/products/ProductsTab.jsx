    ...
    list = [...list].sort((a, b) => {
      switch (sortField) {
        ...
        case "category":
          return (
            dir *
            (a.category_name || a.categories?.name || "").localeCompare(
            ...
            {isLoading ? (
              ...
              paginated.map((p) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-surface-300 hidden sm:table-cell">
                  ...
                  <td className="px-4 py-3 text-right font-mono text-surface-300 hidden md:table-cell">
                    {p.cost_price
                      ? `KSh ${Number(p.cost_price).toLocaleString()}`
                      : "—"}
                  </td>
                  ...
                  <td className="px-4 py-3 text-center">
                    <button
                      onClick={() =>
                        statusMutation.mutate({ id: p.id, status: !p.active })
                      }
                      title="Toggle status"
                    >
                      {p.active ? (
                        ...
                        <ToggleLeft
                          size={20}
                          className="text-surface-500 mx-auto"
                        />
                      )}
                    </button>
                  </td>
                  <td className="px-4 py-3 text-center">
                    <div className="flex items-center justify-center gap-2">
                      <button
                        onClick={() => openEdit(p)}
                        className="p-1.5 hover:bg-surface-600 rounded text-surface-400 hover:text-white transition-colors"
                      >
                        <Edit2 size={14} />
                      </button>
                      <button
            ...
            <form onSubmit={handleSave} className="p-5 space-y-4">
              ...
              <div className="flex gap-3 pt-2">
                <button
                  type="button"
                  onClick={() => setFormOpen(false)}
                  className="flex-1 py-2 bg-surface-700 hover:bg-surface-600 rounded-lg text-sm text-surface-300 transition-colors"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  disabled={isSaving}
                  className="flex-1 py-2 bg-primary-600 hover:bg-primary-700 disabled:opacity-50 rounded-lg text-sm text-white font-medium transition-colors"
                >
